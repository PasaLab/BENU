 ...
 */
public final class GraphWithTimestampDBClient {

    private HBaseGraphMultiVersionStorage distributedGraphStorage;
    public Cache<Integer, int[]> graphCache;
    private double cacheCapacityInGB = 1;

    private static GraphWithTimestampDBClient processLevelStorage = null;
    ...
    private int[] getOldForwardAdj(int vid, int T, boolean isInsert) throws Exception{
        ...
        int[] cacheAdj = graphCache.getIfPresent(vid);
        if (cacheAdj != null) {
            int t = cacheAdj[0];
            if (t == T - 1 && !isInsert) {
    ...
    private int[] generateCombinedAdjInCache(int vid, int[] oldForwardAdj, int[] oldReverseAdj,
