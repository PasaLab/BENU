 ...
 */
public final class EnumeratorWithNoDB implements Serializable {
    public int instructionsNum;
    public int patternVertexNum;
    public int[] type; // types of execution instructions: 0(INI), 1(DBQ), 2(INT), 3(ENU), 4(RES), 5(INS)
    //private int[] INI; // 0:init(start)
    //private AdjType[] adjType;
    public int[] DBQ; // operatorID
    public AdjType[] DBQType; // 0(either), 1(delta), 2(unaltered)
    public AdjType[] DBQDirection; // 0(forward), 1(reverse)
    public int[][] INT; // operator1ID, operator2ID, ...
    public int[] ENU; // OpertorID
    public int[] lastForLoopIndex;
    public int[] matchedVertexCounter;
    public int[][] patternVertexToENUMap; // {{pattern vertex id1, ENU id}, {pattern vertex id2, ENU id}, ...}
    public int[][] injectiveConds;
    public int[][] automorphismConds;
    public int[][] INS; // operator1ID(fx), operator2ID(A?Oy)

    public GraphWithTimestampDBClient graphStorage = null;
    public int timestamp;
    public IntArrayList[] result;

    public IntArrayList filterVertices;

    /**
     * Init the enumeration.
     * @param N number of instructions
     */
    public EnumeratorWithNoDB(int N) {
        instructionsNum = N;
        type = new int[N];
        //INI = new int[N];
    ...
    public EnumeratorWithNoDB(EnumeratorWithNoDB enumerator) {
    ...
    public final void prepare(GraphWithTimestampDBClient graphStorage, Properties conf) {
        try {
    ...
    public static EnumeratorWithNoDB newInstance(EnumeratorWithNoDB enumerator) {
        return new EnumeratorWithNoDB(enumerator);
    }
