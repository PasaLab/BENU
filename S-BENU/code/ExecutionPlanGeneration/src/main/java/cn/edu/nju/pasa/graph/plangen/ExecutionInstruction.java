...
import cn.edu.nju.pasa.graph.plangen.incremental.InstructionNotation;
...
public class ExecutionInstruction {
    ...
    public String toString() {
        ...
        if(type == InstructionType.INI) {
            ...
            String DBQtype = ""; // either/delter/unaltered
            String DBQdirection = ""; // forward/reverse
            switch (type) {
                case DBQ:
                    operation = "GetAdj";
                    if(targetVariable.startsWith(InstructionNotation.DBQ_DELTA_FORWARD_TARGET)) {
                            DBQtype = InstructionNotation.DBQ_TYPE_DELTA;
                            DBQdirection = InstructionNotation.DBQ_FORWARD;
                    }
                    else if(targetVariable.startsWith(InstructionNotation.DBQ_UNALTERED_FORWARD_TARGET)) {
                        DBQtype = InstructionNotation.DBQ_TYPE_UNALTERED;
                        DBQdirection = InstructionNotation.DBQ_FORWARD;
                    }
                    else if(targetVariable.startsWith(InstructionNotation.DBQ_UNALTERED_REVERSE_TARGET)) {
                        DBQtype = InstructionNotation.DBQ_TYPE_UNALTERED;
                        DBQdirection = InstructionNotation.DBQ_REVERSE;
                    }
                    else if(targetVariable.startsWith(InstructionNotation.DBQ_EITHER_FORWARD_TARGET)) {
                        DBQtype = InstructionNotation.DBQ_TYPE_EITHER;
                        DBQdirection = InstructionNotation.DBQ_FORWARD;
                    }
                    else if(targetVariable.startsWith(InstructionNotation.DBQ_EITHER_REVERSE_TARGET)) {
                        DBQtype = InstructionNotation.DBQ_TYPE_EITHER;
                        DBQdirection = InstructionNotation.DBQ_REVERSE;
                    }
                    break;
                ...
                case RES:
                    operation = "ReportMatch"; break;
                case INS:
                    operation = "InSetTest";
            }

            //if(InstructionNotation.DYNAMIC && type == InstructionType.DBQ) {
            if(type == InstructionType.DBQ && !DBQtype.isEmpty()) {
                // incremental execution plan
                if (DBQtype.equals(InstructionNotation.DBQ_TYPE_DELTA)) {
                    instruction.append(targetVariable + ":=" + operation + "(" +
                            singleOperator + "," + DBQtype + "," + DBQdirection + ")");
                } else {
                    instruction.append(targetVariable + ":=" + operation + "(" +
                            singleOperator + "," + DBQtype + "," + DBQdirection + ",op)");
                }
            } else {
                // execution plan
                if(type == InstructionType.INS) instruction.append(operation + "(");
                else if(type == InstructionType.DELTA_ENU) instruction.append("op,"+ targetVariable + ":=" + operation + "(");
                else instruction.append(targetVariable + ":=" + operation + "(");
            ...
            if(!automorphismBreakingConds.isEmpty()) {
                ...
                for(Integer c: automorphismBreakingConds) {
                    ...
                    int vid = Math.abs(c);
                    instruction.append(op + InstructionNotation.ENUTARGET + vid + ",");
                }
            }
            ...
            if(!injectiveConds.isEmpty()) {
                ...
                for(Integer c: injectiveConds) {
                    instruction.append("!" + InstructionNotation.ENUTARGET + c + ",");
                }
            }
